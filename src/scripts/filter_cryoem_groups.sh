...
CRYOEM_BLOBS_DIR="${SCRIPT_DIR}../../data/cryoem_blobs"
RARE_DIR="${CRYOEM_BLOBS_DIR}/RARE"
...
if [[ ! -d "$CRYOEM_BLOBS_DIR" ]]; then
    echo "Error: CryoEM blobs directory not found: $CRYOEM_BLOBS_DIR"
    exit 1
fi
...
echo "CryoEM blobs directory: $CRYOEM_BLOBS_DIR"
echo ""
...
find "$CRYOEM_BLOBS_DIR" -mindepth 1 -maxdepth 1 -type d -print0 | \
    xargs -0 -P "$MAX_WORKERS" -I {} bash -c 'process_folder "$1" "$2" "$3"' _ {} "$TEMP_LIGANDS" "$CRYOEM_BLOBS_DIR" > /tmp/folder_processing.log

# Count results
kept_count=$(grep -c "^keep:" /tmp/folder_processing.log || true)
removed_count=$(grep -c "^removed:" /tmp/folder_processing.log || true)
...
loose_files=$(find "$CRYOEM_BLOBS_DIR" -maxdepth 1 -type f | wc -l)
...
if [[ $loose_files -gt 0 ]]; then
    ...
    find "$CRYOEM_BLOBS_DIR" -maxdepth 1 -type f -exec mv {} "$RARE_DIR/" \;
...
# Clean up
rm -f /tmp/folder_processing.log
...
echo "Summary:"
echo "--------"
echo "Valid ligand folders retained: $(find "$CRYOEM_BLOBS_DIR" -mindepth 1 -maxdepth 1 -type d ! -name "RARE" | wc -l)"
if [[ -d "$RARE_DIR" ]]; then
